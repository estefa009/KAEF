...
<body>
    ...
    <div class="container mt-4">
      <table class="table table-striped">
        ...
        <tbody>
          ...
          $sql = "SELECT entradas.cod_entrada, entradas.cod_insumo, entradas.cnt_entrada, entradas.fecha_caducidad, entradas.fecha_hora_entrada
          FROM entradas 
          INNER JOIN insumo ON entradas.cod_insumo = insumo.cod_insumo";
          ...
          if ($result = $con->query($sql)) {
            if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . htmlspecialchars($row['cod_entrada']) . "</td>";
                echo "<td>" . htmlspecialchars($row['cod_insumo']) . "</td>";
                echo "<td>" . htmlspecialchars($row['cnt_entrada']) . "</td>";
                echo "<td>" . htmlspecialchars($row['fecha_caducidad']) . "</td>";
                echo "<td>" . htmlspecialchars($row['fecha_hora_entrada']) . "</td>";
                echo "<td>
                    <button class='btn btn-danger' title='Eliminar' onclick='confirmDelete(" . htmlspecialchars($row['cod_entrada']) . ")'>
  ...
  <script>
    function confirmDelete(id) {
      const confirmAction = confirm("¿Estás seguro de que deseas eliminar esta entrada?");
      if (confirmAction) {
